...
function initApp() {
  const storedUser = localStorage.getItem('pollify_user');
  if (storedUser) {
    try {
  ...
  allPolls.forEach(poll => {
    ...
    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
        <span class="poll-status-tag ${statusClass}">${statusText}</span>
        <div style="display: flex; gap: 6px; align-items: center;">
          ${scoreIndicatorHtml}
          ${poll.hasVoted ? '<span class="badge voter" style="font-size: 9px; padding: 2px 6px;">Voted</span>' : ''}
        </div>
      </div>
